...
void init_simd_wortel(int n,int lo,int level,double complex root)
{	
	if(n > 1){
		int m = n/2;
...
*
******************************************************************/
void inverse_simd(cplx_ptr *x,int n,int lo,int level)
{	int m = n/2;
	if(n > 8)
	{
...
void recursive_simd(cplx_ptr *x,int n,int lo,int level)
{	
  int m = n/2;	
  if(n > 8)
  {
