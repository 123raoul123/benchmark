...
void init_vec_wortel(int n,int lo,int level,double complex root)
{	
	if(n > 1){
		int m = n/2;
		...
		++level;
		init_vec_wortel(m,lo,level,csqrt(root));
		init_vec_wortel(m,lo+m,level,csqrt(-root));
	}
}
...
{	
	...
	init_vec_wortel(CPLXDIM,0,0,csqrt(I));
}
...
{	
  ...
  else if(n == 8)
  {
	...
	real_twid = _mm256_setr_pd(vec_wortel[0][level][lo/4],vec_wortel[0][level][lo/4],vec_wortel[0][level][(lo+m)/4],vec_wortel[0][level][(lo+m)/4]);
    imag_twid = _mm256_setr_pd(vec_wortel[1][level][lo/4],vec_wortel[1][level][lo/4],vec_wortel[1][level][(lo+m)/4],vec_wortel[1][level][(lo+m)/4]);
	...
	// // //START LAYER 2!!
	++level;
	real_twid = _mm256_setr_pd(vec_wortel[0][level][lo/2],vec_wortel[0][level][(lo+2)/2],vec_wortel[0][level][(lo+m)/2],vec_wortel[0][level][(lo+m+2)/2]);
    imag_twid = _mm256_setr_pd(vec_wortel[1][level][lo/2],vec_wortel[1][level][(lo+2)/2],vec_wortel[1][level][(lo+m)/2],vec_wortel[1][level][(lo+m+2)/2]);
...
void vec_rec_forward(cplx_ptr *x,const ring_t *ring)
{
  int j = CPLXDIM;
...
void vec_rec_backward(cplx_ptr *x, ring_t *ring)
{
